...
def exponential_backoff(attempt: int, base_delay: int, max_delay: int) -> None:
    """Sleep using exponential backoff strategy."""
...
def fetch_gas_prices(retries: int = RETRY_LIMIT, delay: int = RETRY_DELAY) -> Optional[Dict[str, Optional[float]]]:
    ...
    for attempt in range(retries):
        try:
            ...
            logger.warning("Network-related error: %s [Attempt %d/%d]", e, attempt + 1, retries)
        except Exception as e:
            logger.error("Unexpected error: %s [Attempt %d/%d]", e, attempt + 1, retries)
...
def monitor_gas_prices(interval: int = MONITOR_INTERVAL) -> None:
    """Continuously monitors and logs gas prices until interrupted."""
    logger.info("Started gas price monitoring every %ds.", interval)
    while running:
